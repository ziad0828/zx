...
ajex = Token:match("(%d+)")
redis:set(ajex..":VERSION",1)
redis:set(ajex..":SUDO_ID:",SUDO_USER)
redis:set(ajex..":DataCenter:",'German')
redis:set(ajex..":UserNameBot:",BOT_User)
redis:set(ajex..":NameBot:",BOT_NAME)
redis:hset(ajex..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3ajex_INSTALL","Yes")
info = {}
...
ajex = Token:match("(%d+)")
our_id = tonumber(ajex)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(ajex..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ajex..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ajex..'username:'..SUDO_ID).username
version = redis:get(ajex..":VERSION")
DataCenter = redis:get(ajex..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ajex..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ajex..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ajex..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ajex..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(ajex..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(ajex..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(ajex..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(ajex..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(ajex..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(ajex..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(ajex..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(ajex..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(ajex..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(ajex..'username:'..SUDO_ID).username
...
if msg.GroupActive and redis:get(ajex..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(ajex..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ajex..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ajex..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ajex then 
if msg.text and ScriptFile.iajex then
for k, ajex in pairs(ScriptFile.ajex) do
local SearchText = msg.text:match(ajex)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iajex(msg,{msg.text:match(ajex)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ajex.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dajex then
if ScriptFile.dajex(msg) == false then
return false
end
...
if Plug.ajex then 
if msg.text and Plug.iajex then
for k, ajex in pairs(Plug.ajex) do
local SearchText = msg.text:match(ajex)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(ajex..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(ajex..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iajex(msg,{msg.text:match(ajex)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ajex.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source ajex")
end 

end
else
print("The File Script.lua Not Runing in The Source ajex")
...
function tdcli_update_callback(data)
	...
	redis:srem(ajex..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/Ajjexx/ajex/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(ajex..":SUDO_ID:")
redis:hset(ajex..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(ajex..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(ajex..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ajex..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ajex..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ajex..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ajex..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ajex..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(ajex..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(ajex..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(ajex..'group:add'..data.chat_.id_) then
	redis:set(ajex..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(ajex..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(ajex..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(ajex..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	ajex..'group:name'..k,IDS.Title,
	ajex..'num_msg_ajex'..k,5,
	ajex..'group:add'..k,true,
	ajex..'lock_link'..k,true,
	ajex..'lock_spam'..k,true,
	ajex..'lock_webpage'..k,true,
	ajex..'lock_markdown'..k,true,
	ajex..'lock_flood'..k,true,
	ajex..'lock_bots'..k,true,
	ajex..'mute_forward'..k,true,
	ajex..'mute_contact'..k,true,
	ajex..'mute_document'..k,true,
	ajex..'mute_inline'..k,true,
	ajex..'lock_username'..k,true,
	ajex..'replay'..k,true
	)
	redis:sadd(ajex..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ajex..'username:'..ID,'username',user)
	redis:sadd(ajex..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ajex..'username:'..ID,'username',user)
	redis:sadd(ajex..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ajex..'username:'..ID,'username',user)
	redis:sadd(ajex..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'⌯ ** تم رفع النسخه‏‏ الاحتياطيه\n⌯ ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(ajex..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(ajex..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ajex..'username:'..data.user_.id_).username
	...
	redis:hset(ajex..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
